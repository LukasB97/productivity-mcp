...
2. Google authorization writes one encrypted refresh token with the account's selected scopes to its account-specific directory. Its encryption key is held by the operating-system credential store. Adding Gmail requests the combined selected scopes in a staging directory and verifies the account identity before replacing a working token. The account catalog keeps only the local account key, email address, and enabled-service flags.
