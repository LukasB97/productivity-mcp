...
public sealed class MultiGoogleEmailProvider(GoogleOptions options, EmailContentService content) : IEmailProvider
{
    ...
    public Task<OperationResult<EmailMessage>> ForwardAsync(string account, string id, IReadOnlyList<string> to, IReadOnlyList<string> cc, IReadOnlyList<string> bcc, EmailComposeBody? body, CancellationToken token = default) => Run(account, p => p.ForwardAsync(id, to, cc, bcc, body, token));
